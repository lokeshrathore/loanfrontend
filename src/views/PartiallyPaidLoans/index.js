...
const PartiallyPaidLoansLoans = () => {
  const userId = localStorage.getItem('user_id');

  const [partiallyLoanData, setPartiallyLoanData] = useState([]);

  const getPartiallyLoanList = async () => {
    const response = await getApi(`loan/list/partially?createdBy=${userId}`);
    if (response && response.status === 200) {
      setPartiallyLoanData(response.data.loans);
    }
  };
  useEffect(() => {
    getPartiallyLoanList();
  }, []);
    ...
    <>
      <Container>
        <Stack direction="row" alignItems="center" mb={5} justifyContent={'space-between'}>
          <Typography variant="h4">Partially-Paid-Loans-Lists</Typography>
        </Stack>
        <TableStyle>
          <Box width="100%">
            <Card style={{ height: '600px', paddingTop: '15px' }}>
              <DataGrid
                rows={partiallyLoanData ?? []}
                columns={columns}
...
export default PartiallyPaidLoansLoans;
