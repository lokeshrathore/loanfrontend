...
const DeniedLoans = () => {
  ...
  const [deniedLoanData, setDeniedLoanData] = useState([]);

  const getDeniedLoanList = async () => {
    const response = await getApi(`loan/list/denied?createdBy=${userId}`);
    if (response && response.status === 200) {
      setDeniedLoanData(response.data.loans);
    }
  };
  useEffect(() => {
    getDeniedLoanList();
    ...
    <>
      <Container>
        <Stack direction="row" alignItems="center" mb={5} justifyContent={'space-between'}>
          <Typography variant="h4">Denied-Loans-Lists</Typography>
        </Stack>
        <TableStyle>
          <Box width="100%">
